...
def main():
    '''testaa miller-rabinia n kertaa 1024 bit random -komposiitilla'''
    n_value = 100
    test_passed = True
    for _ in range (n_value):
        ...
        if RsaService().miller_rabin_check(a_value * b_value) is True:
            print("Miller-rabin teki virheen syötteellä: \n", a_value * b_value)
            test_passed = False
            break
    if test_passed:
        print("Miller Rabin tuotti oikean tuloksen joka kerta. Testattiin ",n_value,"kertaa")
